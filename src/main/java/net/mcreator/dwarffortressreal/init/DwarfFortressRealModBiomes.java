...
import net.mcreator.dwarffortressreal.world.biome.GOGOPOWERRANGERSBiome;
import net.mcreator.dwarffortressreal.world.biome.GBiome;
...
public class DwarfFortressRealModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, DwarfFortressRealMod.MODID);
	public static final RegistryObject<Biome> G = REGISTRY.register("g", () -> GBiome.createBiome());
	public static final RegistryObject<Biome> GOGOPOWERRANGERS = REGISTRY.register("gogopowerrangers", () -> GOGOPOWERRANGERSBiome.createBiome());

	@SubscribeEvent
	public static void init(FMLCommonSetupEvent event) {
		event.enqueueWork(() -> {
			GBiome.init();
			GOGOPOWERRANGERSBiome.init();
		});
	}
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
						...
						parameters.add(new Pair<>(GBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, G.getId()))));
						parameters.add(new Pair<>(GOGOPOWERRANGERSBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, GOGOPOWERRANGERS.getId()))));
						parameters.add(new Pair<>(GOGOPOWERRANGERSBiome.PARAMETER_POINT_UNDERGROUND,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, GOGOPOWERRANGERS.getId()))));
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							...
							surfaceRules.add(1,
									anySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, GOGOPOWERRANGERS.getId()),
											DwarfFortressRealModBlocks.POWERRANGING.get().defaultBlockState(),
											DwarfFortressRealModBlocks.TROLLING.get().defaultBlockState(),
											DwarfFortressRealModBlocks.POWERRANGING.get().defaultBlockState()));
							surfaceRules.add(1,
									preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, G.getId()),
											Blocks.JUNGLE_LEAVES.defaultBlockState(), Blocks.WEATHERED_COPPER.defaultBlockState(),
											Blocks.WEATHERED_COPPER.defaultBlockState()));
							surfaceRules.add(1,
									preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, GOGOPOWERRANGERS.getId()),
											DwarfFortressRealModBlocks.POWERRANGING.get().defaultBlockState(),
											DwarfFortressRealModBlocks.TROLLING.get().defaultBlockState(),
											DwarfFortressRealModBlocks.POWERRANGING.get().defaultBlockState()));
